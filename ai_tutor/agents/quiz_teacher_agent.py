...
def quiz_user_answers_handoff_filter(handoff_data: HandoffInputData) -> HandoffInputData:
    ...
    """
    print("Applying handoff filter to pass quiz and user answers to quiz teacher agent")
    print(f"HandoffInputData type: {type(handoff_data)}")
    
    try:
        # Use a very conservative max_decimal_places value
        processed_data = round_search_result_scores(handoff_data, max_decimal_places=5)
        print("Applied score rounding to handoff data")
        
        # Extra validation - try to serialize to JSON and back
        if hasattr(processed_data, 'data') and processed_data.data:
            try:
                json_str = json.dumps(str(processed_data.data))
                json.loads(json_str)
