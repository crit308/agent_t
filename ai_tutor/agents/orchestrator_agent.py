...
def create_orchestrator_agent(api_key: str = None) -> Agent['TutorContext']:
    ...
    orchestrator_tools = [
        # call_quiz_creator_mini, # Optionally keep/remove based on desired flow
        call_quiz_teacher_evaluate,
        determine_next_learning_step, # Use the new tool
        update_user_model,
        get_user_model_status,
        update_explanation_progress,
    ]
    ...
    orchestrator_agent = Agent['TutorContext'](
        ...
        Your primary goal is to help the user achieve the learning objectives for the current section.
        ...
        CORE WORKFLOW:
        1.  **Assess State & Objectives:** Check user input, `UserModelState` (especially mastery related to `current_section_objectives`), pace, style, etc.
        ...
        3.  **Determine Next Step (if no pending interaction):**
            *   **If current topic objectives ARE MET** (check `mastered_objectives_current_section` vs `current_section_objectives`): Call `determine_next_learning_step` to get the *next* topic and its objectives. If a next topic exists, signal to initiate teaching for it (segment 0). If lesson complete, send a completion message.
            *   **If current topic objectives ARE NOT MET:** Decide the *micro-step* to work towards the *next unmet objective* in `current_section_objectives`.
                ...
                - If the user struggled: Decide whether to re-explain (signal initiate teaching for the *same segment*), provide a hint (`MessageResponse`), or ask a checking question (`call_quiz_creator_mini`) focused on the specific objective/concept they struggled with.
            *   **If NO current topic:** Call `determine_next_learning_step` to start the first topic. Signal initiate teaching.
            *   **Continuing Topic:**
        ...
        4.  **Select Action & Update State:**
            ...
            *   **Feedback:** Use `call_quiz_teacher_evaluate`. Your output *is* the tool's `FeedbackResponse` or an error message.
        ...
        5.  **Formulate Response:** Your final output for this turn **MUST** be a JSON object matching one of the allowed types in the `TutorInteractionResponse` schema:
            *   If calling a tool like quiz creator/evaluator: The direct output of that tool (`QuestionResponse`, `FeedbackResponse`, or an error string wrapped in `ErrorResponse`).
